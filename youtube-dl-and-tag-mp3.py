...
regex_Provided_to_Youtube_videoDetails = '"videoDetails":\{"videoId":"[a-zA-Z0-9_\-]+","title":".+","lengthSeconds":"[0-9]*","keywords":\[.*\],"channelId":"[a-zA-Z0-9_\-]+","isOwnerViewing":.+,"shortDescription":"Provided to YouTube (.+)Auto-generated by YouTube.","isCrawlable":.+,"thumbnail":\{"thumbnails":\[\{"url":'	# work on line 20
...
def scrapForMetatagsTableFormat(line):
    ...
    match = re.search(regex_TableFormat1, line)
    if match:
        #print("scrapForMetatagsTableFormat: regex1 matches at position ", match.start())
        result = {}
        ...
        match = re.search(regex_TableFormat2, line)
        if match:
            #print("scrapForMetatagsTableFormat: regex2 matches at position ", match.start())
            result = {}
            result["title"]     = match.group(1)
            result["artist"]    = match.group(2)
            result["album"]     = ""		# no album here
            result["label"]     = ""
            result["year"]      = ""
            result["company"]   = ""
            return result
